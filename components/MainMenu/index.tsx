...
const Index: React.FC<IndexProps> = ({ isMainMenuOpen, navigateTo, currentPage }) => {
  ...
  return (
    <div
      className={`
        fixed z-10 left-1/2 -translate-x-1/2 w-[50vw] h-10  rounded-full bg-gray-800 text-white 
        flex items-center justify-center gap-6 
        transform transition-transform duration-300
        ${isMainMenuOpen ? 'translate-y-0 top-1' : '-translate-y-full top-0'}
      `}
      >
      <UserIcon 
      ...
      <DevicePhoneMobileIcon 
        className={`w-6 h-6 text-white ${currentPage === 'strength' ? 'opacity-100' : 'opacity-40'}`}
        onClick={() => handleClick('strength')}
        />
      <PaintBrushIcon className={`w-6 h-6 text-white ${currentPage === 'bodychart' ? 'opacity-100' : 'opacity-40'}`}/>
    </div>
  );
};
